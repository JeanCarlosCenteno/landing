...
const CategoryComponent = () => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (editCategoryId) {
        ...
        await axios.put(`http://localhost:3000/category/${editCategoryId}`, updatedCategory);
  ...
  return (
    <div>
      <h1 className="text-lg font-bold text-violet-600">Listas de Categorías</h1>
      <div>
        ...
        <div className="border border-gray-300 p-4 rounded-md">
          {categories.map((item: Category) => (
            <div key={item.id} className="flex items-center justify-between mb-4">
              <div>
                <h3 className="text-lg font-semibold">Descripción: {item.descriptionProduct}</h3>
              </div>
              <div>
                <button
